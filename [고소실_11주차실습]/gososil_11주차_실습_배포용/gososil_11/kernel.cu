...
//	printf("blockIdx.y : %d, blockIdx.x : %d\n", blockIdx.y, blockIdx.x);
	...
	for (int i = 0; i < ELEM_PER_VECTOR; i++) {
		...
		for (int j = 0; j < ELEM_PER_VECTOR; j++) {
			tmp += d_MatA[i*ELEM_PER_VECTOR + j] * d_VecX[id * ELEM_PER_VECTOR + j];
		}
		d_VecY[id * ELEM_PER_VECTOR + i] = tmp;
	}
}
...
void Mat_Vec_Multiply_GPU(float *p_VecX, float *p_MatA, float *p_VecY_G)
{
	...
	CUDA_CALL(cudaMemcpy(d_A, p_MatA, size, cudaMemcpyHostToDevice))

	size = n * ELEM_PER_VECTOR * sizeof(float);
	CUDA_CALL(cudaMalloc(&d_X, size))
	CUDA_CALL(cudaMemcpy(d_X, p_VecX, size, cudaMemcpyHostToDevice))
	...
	CHECK_TIME_INIT_GPU()
	CHECK_TIME_START_GPU()
	Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (d_Y, d_X, d_A, ELEM_PER_VECTOR * sizeof(float));
	CHECK_TIME_END_GPU(device_time)
	CHECK_TIME_DEST_GPU()
...
int main()
{
    ...
    for (i = 0; i < ELEMENT_SIZE * ELEM_PER_VECTOR; i++) {
		//printf("pVecY[%d] : %d, pVecY_G[%d] : %d\n", i, pVecY[i], i, pVecY_G[i]);
