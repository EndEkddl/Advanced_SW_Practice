...
void CColorConversionView::OnProblem1Sub1()
{
	...
	for (r = 0; r < width*height; r++) {
		mg_lpImage[r] = m_dibFile.m_lpImage[r] / 2;
	}
...
void CColorConversionView::OnProblem1Sub2()
{
	...
	for (r = 0; r < width*height; r++) {
		if (m_dibFile.m_lpImage[r] * 2 > 255) {
			mg_lpImage[r] = 255;
		}
		else mg_lpImage[r] = m_dibFile.m_lpImage[r] * 2;
	}
...
void CColorConversionView::OnProblem2()
{
	...
	for (c = 0; c < width*height; c++) {
		mg_lpImage[c] = m_dibFile.m_lpImage[3 * c] * 0.114 + m_dibFile.m_lpImage[3 * c + 1] * 0.587 + m_dibFile.m_lpImage[3 * c + 2] * 0.299;
	}
...
void CColorConversionView::OnProblem3()
{
	...
	for (c = 0; c < width*height; c++) {
		rd = (tempim16[c] >> 10) & 0x1f;
		gr = (tempim16[c] >> 5) & 0x1f;
		bl = (tempim16[c]) & 0x1f;
		mg_lpImage[c] = (bl * 0.114 + gr * 0.587 + rd * 0.299) * 255 / 31;
	}
	m_width = width;
	m_height = height;
