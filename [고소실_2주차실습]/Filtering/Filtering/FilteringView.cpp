...
void CFilteringView::OnProblem3()
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for (l = -w; l <= w; l++) {
				for (k = -w; k <= w; k++) {
					if (r + l >= 0 && r + l < m_height) {
						Gy[r * m_width + c] += Sy[l + w][k + w] * m_imageBits[(r + l) * m_width + c + k];
					}
					if (c + k >= 0 && c + k < m_width) {
						Gx[r * m_width + c] += Sx[l + w][k + w] * m_imageBits[(r + l) * m_width + c + k];
					}
				}
			}
...
void CFilteringView::AverageFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for (l = -w; l <= w; l++) {
				for (k = -w; k <= w; k++) {
					if ((r + l < 0) || (c + k < 0) || (r + l >= m_height) || (c + k >= m_width)) continue;
					mean += imageBits[(r + l) * (m_width)+(c + k)] * (double)(1 / (double)w0 / w0);
				}
			}
			

			imageBitsFiltered[r*m_width+c] = (int)mean;
		}
	}
	QueryPerformanceCounter(&m_end);
	m_result = 1000*(m_end.QuadPart-m_start.QuadPart)/m_frequency.QuadPart;	
		...
		for (c=0; c<m_width; c++) {
			...
			if (!c) {
				...
				for (l = -w; l <= w; l++) {
					for (k = -w; k <= w; k++) {
						if ((r + l < 0) || (c + k < 0) || (r + l >= m_height) || (c + k >= m_width)) continue;
						mean += m_imageBits[(r + l) * (m_width)+(c + k)] * (double)(1 / (double)w0 / w0);
					}
				}
			}
			else {
				for (l = -w; l <= w ; l++) {
					if (r + l < 0 || r + l >= m_width) continue;
					if (c - w - 1 >= 0 && c - w - 1 < m_width) {
						mean -= m_imageBits[(r + l) * m_width + c - w - 1] * (double)(1 / (double)w0 / w0);
					}
					if (c + w >= 0 && c + w < m_width) {
						mean += m_imageBits[(r + l) * m_width + c + w] * (double)(1 / (double)w0 / w0);
					}
				}
			}
		...
		for (c=0; c<m_width; c++) {
			...
			for (l = -w; l < w; l++)
			{
				for (k = -w; k < w; k++)
				{
					...
					if ((r + l >= 0) && (c + k >= 0) && (r + l < m_height) && (c + k < m_width))
						medarray[idx++] = m_imageBits[(r + l) * m_width + (c + k)];
					else
						medarray[idx++] = 0;
				}
			}
