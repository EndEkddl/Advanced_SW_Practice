				...
				for (int x = 0; x < size; x++) {
					for (int ky = 0; ky < conv->k_size; ky++) {														//이때 한 점은 kernel_size * kernel_size 번의 덧셈을 통해 계산
						for (int kx = 0; kx < conv->k_size; kx++) {
							...
							////////////////////////////////////////////////////////////////////////////////
							/* ADD YOUR CODE HERE */
							//int x_index = x + kx - conv->k_size / 2 + 1;									
							//int y_index = y + ky - conv->k_size / 2 + 1;
								tmp[y * conv->f_size + x] += input2D[(y + ky)*input_size + (x + kx)] * kernel2D[ky * conv->k_size + kx];
							//	tmp[y * conv->f_size + x] += kernel2D[ky * conv->k_size + kx] * input2D[y_index * conv->f_size + x_index];
						}
					}
				}
			}
			...
			for (int y = 0; y < pool->f_size; y++) {
				for (int x = 0; x < pool->f_size; x++) {
					for (int ky = 0; ky < conv->k_size; ky++) {
						for (int kx = 0; kx < conv->k_size; kx++) {
							int x_index = x + kx - conv->k_size/2 + 1;									//conv layer의 error에서의 x index
							int y_index = y + ky - conv->k_size/2 + 1;									//conv layer의 error에서의 y index
